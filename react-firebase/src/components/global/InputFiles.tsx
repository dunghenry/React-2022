...
const InputFiles:React.FC<IProps> = ({multiple, files, setFiles}) => {
    ...
    const checkImages = (files: FileList) => {
        ...
        Array.from(files).map(file => {
            if (!file) return toast.error("File does not exits.");
                                ...
                                files.map((file, index) => {
                                    return (
                                        <div key={index} className="flex flex-col items-center">
                                            <img src={URL.createObjectURL(file)} alt="image-upload" className="object-cover w-12 h-12 mx-1 border rounded-full" />
                                        </div>
                                    )
                                })
                            }
